...
- **Não se testa wrapper de bibliotheca de terceiro.** Docling, Gradio e sentence-transformers são deles. O que se testa é o código do projeto.
...
| `biblio/paths.py` | Onde a bibliotheca mora (`~/biblio`), registro das bibliothecas conhecidas, slug de nome de documento |
...
| `biblio/index.py` | Gera `INDEX.md` e `CLAUDE.md` a partir da bibliotheca em disco |
...
| `tests/conftest.py` | Fixtures: PDFs sintéticos, bibliotheca de brinquedo |
...
### Tarefa 0.0: Ensaio do protocolo com bibliotheca falsa **[sem TDD]**

**Files:**
- Create: `ensaio/bibliotheca-falsa/` (descartável, fora do pacote; apague ao terminar)
...
- [x] **Step 1: Forjar o mínimo que o agente veria**

Escreva à mão, sem nenhum código do `biblio`:

1. `~/.claude/skills/bibliotheca/SKILL.md` — o texto da Tarefa 2.4 copiado à mão, com `description` nomeando uma bibliotheca falsa.
2. `ensaio/bibliotheca-falsa/` com **quatro** arquivos `.md` fatiados de verdade, tirados de um documento real do seu acervo (recorte à mão), cada um com o frontmatter da spec §5.1, mais um `INDEX.md` e um `CLAUDE.md`.
...
- [x] **Step 2: Abrir um Claude Code novo e perguntar**
...
Repita com uma segunda pergunta, agora **apontando** a pasta `ensaio/bibliotheca-falsa/`, e confira se ele usa `--lib`.

- [x] **Step 3: Decidir com o resultado na mão**
...
#### Resultado da primeira rodada (2026-09-07)

**Claude Code, sessão nova, nada apontado: premissa de pé.** A skill disparou sozinha, o agente reescreveu a pergunta em termos do domínio, buscou, leu e respondeu **1,35** — o número que só existe dentro do arquivo. Trouxe de brinde as ressalvas de 5.3, 3.1 e 5.4.

Dois defeitos reais, os dois já corrigidos acima:

1. **`biblio: command not found` na primeira chamada.** O shell POSIX do Claude Code recebe todo o PATH do Windows como **uma única entrada** — nada dele resolve, nem `ls`, nem `grep`. O agente se recuperou trocando para o PowerShell, mas gastou duas chamadas. Daí `_executavel()`: a skill grava o caminho absoluto, que ela conhece na hora de se instalar.
2. **Leu 1-40 em vez de 19-36.** O único exemplo do protocolo era `:1-84 → offset=1, limit=84`, onde `limit` coincide com a linha final. Ensinava a aritmética errada. Agora o exemplo começa fora do 1 e a regra está escrita: `limit = fim - inicio + 1`.

**Cowork, sessão nova, nada apontado: não disparou nada.** Foi direto para busca na web. É o comportamento previsto — o Cowork não carrega skill do Claude Code, e o `CLAUDE.md` só entra quando alguém aponta a pasta.

Mas falsifica a afirmação de que "os dois veículos cobrem todos os casos" (spec §7.3): **Cowork sem apontar não tem veículo nenhum.** Fica registrado como limitação declarada, não como algo a resolver com um terceiro mecanismo — apontar a pasta é um gesto, e é o preço de um ambiente que não carrega skills.

#### Resultado da segunda rodada (2026-09-07)

**Claude Code: limpo.** Uma chamada de busca pelo caminho absoluto, sem `command not found`. Leu `19,36` e `10,14` — os intervalos exatos, sem arredondar. Respondeu 1,35 e ainda avisou que a fonte era a NI-4471 e não a NBR 9062 que a pergunta citava. **Os dois defeitos da rodada 1 estão fechados, e a premissa do projeto está validada com evidência.**

**Cowork, com a pasta apontada: leu tudo.** Listou a pasta recursiva e deu um `cat` em **todos os arquivos** — `CLAUDE.md`, `INDEX.md` e as quatro fatias — numa única chamada. Respondeu certo, mas por varredura.

O detalhe que importa: **o Cowork tinha shell.** Rodou `bash` e `cat`; poderia ter rodado `biblio search`. Não foi falta de capacidade.

**O `CLAUDE.md` chegou tarde.** Ele foi lido *dentro* da mesma varredura que existia para impedir — o agente leu "**não a leia por varredura**" enquanto varria. No Claude Code a skill entra em contexto **antes** da primeira ação e por isso molda a primeira ação; no Cowork o `CLAUDE.md` é só mais um arquivo da pasta, e só é lido quando a pasta já está sendo lida.

Isto não se conserta com um terceiro mecanismo nem com redação melhor: é a diferença entre um host que carrega instrução antes de agir e um que não carrega. Duas consequências, ambas registradas:

1. **Onde não há carregamento prévio, o produto é a pasta fatiada + `INDEX.md`, não a busca.** Neste ensaio a bibliotheca tinha 6 KB e varrer saiu barato — que é exatamente o caso em que o `biblio` não faz falta. Com 200 documentos, varrer deixa de ser possível, e o `INDEX.md` vira a porta de entrada por necessidade. Ele passa a dizer isso explicitamente (Tarefa 2.4).
2. **Um pedido explícito resolve.** "Use `biblio search` nessa pasta" em vez de "confira essa pasta" devolve o protocolo inteiro, porque a instrução chega antes. É uma frase, não um recurso.

- [x] **Step 4: Commit**

```bash
git add ensaio/ && git commit -m "chore: ensaio do protocolo com bibliotheca falsa"
```
...
- [x] **Step 1: Criar o ambiente conda**
...
- [x] **Step 2: Escrever o `pyproject.toml`**
...
- [x] **Step 3: Criar os pacotes vazios**
...
- [x] **Step 5: Verificar que a suíte roda vazia**
...
- [x] **Step 1: Testar se `sqlite-vec` sequer carrega**
...
- [x] **Step 2: Medir a força bruta em numpy no pior caso plausível**
...
1. **Descarte quem errar qualquer um dos quatro casos entre idiomas.** Este é o critério duro: um modelo que só acerta dentro do mesmo idioma faz a bibliotheca mista responder pela metade, e responder pela metade parece funcionar.
...
- [x] **Step 4: Fixar a escolha em código**
...
- [x] **Step 5: Registrar no spec e commitar**
...
**Contexto:** wrapper de bibliotheca de terceiro. O único código próprio aqui é a **intercalação por página** e o marcador `<!-- pag N -->`, que é o que faz o `slice` conseguir preencher o campo `paginas` do frontmatter sem uma segunda passada pelo PDF.
...
> Ajuste na implementação: `pymupdf4llm` 0.0.17+ instalado nomeia a chave de página
> como `page_number` (1-based), não `page`. Uma linha em `_converter_nativas`.
...
- [x] **Step 2: Verificação de fumaça num PDF nativo real**
...
- [x] **Step 1: Escrever `paths.py` primeiro (o `slice` precisa do slug)**
...
- [x] **Step 6: Rodar a suíte inteira (nada quebrou para trás)**
...
- [x] **Step 2: Verificação de fumaça**
...
**Files:**
- Modify: `biblio/paths.py` (registro das bibliothecas conhecidas)
...
- [x] **Step 1: Registro de bibliothecas em `paths.py`**

O usuário vai criar mais de uma bibliotheca ao longo do tempo. Sem registro, a busca só enxergaria a padrão e a segunda falharia em silêncio.
...
- [x] **Step 2: Implementar o pipeline**
...
def adicionar(alvo: Path | str, saida: Path | str | None = None, device: str = "auto",
    ...
    for arquivo in _arquivos(Path(alvo)):
        ...
        try:
            contagem[_processar_um(arquivo, bibliotheca, device, force, avisar)] += 1
...
- [x] **Step 3: Verificação de fumaça — uma pasta com PDF e MD juntos**
...
biblio.paths.REGISTRO = entrada / 'bibliothecas.txt'  # nao suja o registro real
...
assert todas('lib')[0] == lib.resolve(), 'nome de bibliotheca conhecida'
assert todas('nao-existe')[0] == pathlib.Path('nao-existe'), 'nome desconhecido volta como veio'
"
```

Note a **primeira linha do script**: ela desvia `REGISTRO` para a pasta temporária, senão esta fumaça suja o seu registro real com uma bibliotheca de teste. O mesmo desvio vira fixture `autouse` na Tarefa 4.2.
...
```bash
git add biblio/paths.py biblio/pipeline.py && git commit -m "feat: ingestao de pdf, md e txt com registro de bibliotheca"
```
...
Isso **não** quer dizer deixar o usuário se virar: a ferramenta instala, ela só não instala escondido. O consentimento da GUI é dado antes de começar, numa caixa marcada por padrão que fala do resultado ("gerar resumo de cada documento") e revela o custo ("baixa ~3 GB na primeira vez") — e não numa pergunta no meio do processamento sobre um programa cujo nome o usuário nunca ouviu.

- [x] **Step 1: Implementar `ollama.py`**
...
- [x] **Step 2: Implementar `summarize.py`**
...
- [x] **Step 4: Verificar que o pipeline sobrevive ao Ollama ausente**
...
### Tarefa 2.4: `skill.py` e `index.py` — a bibliotheca se explica sozinha **[sem TDD]**
...
Os invólucros diferem num campo, `{escopo}`: a skill busca em todas as bibliothecas registradas, o `CLAUDE.md` traz `--lib` da própria pasta. É isso que faz **apontar o Claude para uma pasta significar consultar aquela bibliotheca** (spec §6.0) — sem isso, o `CLAUDE.md` ensinaria a buscar no acervo inteiro da máquina, e o usuário que separou álgebra linear de controle digital receberia os dois de volta.

- [x] **Step 1: Escrever `skill.py`**
...
- [x] **Step 3: Verificação de fumaça**
...
assert '--lib \"' not in texto_skill(), 'a skill global nao pode nascer presa a uma bibliotheca'
" <caminho da lib>
```
...
```bash
git add biblio/skill.py biblio/index.py && git commit -m "feat: INDEX.md e CLAUDE.md gerados na bibliotheca"
```
...
- [x] **Step 2: Verificação de fumaça do schema**
...
- [x] **Step 3: Verificação de fumaça — o ponteiro tem que ser exato**
...
- [x] **Step 1: Escrever a fixture do corpus**
...
def buscar(consulta: str, saida=None, top: int = 5, doc: str | None = None) -> list[dict]:
    ...
    achados: list[dict] = []
    vistos: set[str] = set()
    for (bibliotheca, identificador), ponto in sorted(rrf(listas).items(),
                                                     key=lambda p: -p[1]):
...
> **Concluída (2026-09-07).** `cli.py`, `gui.py`, `shortcut.py`, `test_idempotencia.py`
> e `README.md` implementados; suíte em `39 passed`. A idempotência já estava correta
> (teste 5 passou de primeira). Dois passos de verificação manual ficaram pendentes por
> exigirem interação/arquivos do usuário: **4.3 Step 2** (arrastar PDF+MD na GUI e
> conferir o painel final) e **4.5 Step 3** (aceitação com documento real do acervo).
> Componentes da GUI validados contra Gradio 6.26.0 (o plano assumia ≥4.44).
...
- [x] **Step 2: Verificar os subcomandos**
...
- [x] **Step 1: Isolar o registro de bibliothecas**

`adicionar()` registra a bibliotheca em `~/.biblio/bibliothecas.txt`. Sem isto, rodar a suíte suja o registro real do usuário — e depois `biblio search` varreria pastas de `tmp` que já não existem.
...
**O botão diz "Adicionar documentos", não "Criar bibliotheca"** (spec §7.2). O uso real é incremental; um botão que diz "criar" treina o usuário a fazer uma bibliotheca nova a cada rodada, e aí ele tem cinco e procura em nenhuma. Criar outra continua sendo digitar outro nome no campo do topo.
...
- [x] **Step 2: Verificar que sobe e fecha o ciclo**

```bash
/c/Users/Usuario/.conda/envs/biblio/Scripts/biblio.exe gui
```

Esperado: navegador abre em `http://127.0.0.1:7860`. Arraste um PDF **e um `.md`** juntos, clique **Adicionar documentos**, veja o progresso linha a linha. Ao terminar, **o painel tem que mostrar o caminho da bibliotheca e um `biblio search` de exemplo com um termo real** do que acabou de entrar — copie esse comando, rode no terminal e confirme que devolve ponteiros. `Ctrl+C` para encerrar.

Confira também o `FileExplorer`: ele tem que **listar pastas** e devolver um caminho utilizável. Se o `glob="**/"` não listar diretórios nesta versão do Gradio, volte ao `gr.Textbox` de caminho e siga — é componente de terceiro, e um campo de texto que funciona vale mais que um seletor que não abre.
...
- [x] **Step 2: Criar o atalho e conferir que a skill foi junto**

```bash
/c/Users/Usuario/.conda/envs/biblio/Scripts/biblio.exe shortcut && /c/Users/Usuario/.conda/envs/biblio/python.exe -c "
import pathlib
alvo = pathlib.Path.home() / '.claude' / 'skills' / 'bibliotheca' / 'SKILL.md'
...
Nada a fazer: a skill se instala em `~/.claude/skills/bibliotheca/` no
`biblio shortcut` e a cada ingestao bem-sucedida. Com ela, `biblio search` cobre
todas as bibliothecas conhecidas desta maquina.

Levar uma bibliotheca para outro computador e copiar a pasta. Nenhum caminho
...
**Para um projeto usar so uma bibliotheca**, aponte o caminho da pasta para o
...
Custo em contexto de uma sessao que consulta a bibliotheca tres vezes: ~900 tokens.
Uma unica pagina de PDF enviada nativamente para a API custa mais que isso.
```

- [x] **Step 2: Confirmar que a instalação está mesmo automática**
...
alvo = pathlib.Path.home() / '.claude' / 'skills' / 'bibliotheca'
...
from biblio.paths import conhecidas_bibliotheca
for caminho in conhecidas_bibliotheca()[:8]:
...
- [x] **Step 3: Aceitação com material real do acervo**
...
6. Uma segunda bibliotheca, criada noutra rodada com `--out`, aparece em `biblio libs` e **é alcançada pela busca sem nenhuma flag**.

**O fluxo do usuário**

7. O atalho na área de trabalho abre a GUI sem console preto; processar por ela dá o mesmo resultado que pela CLI.
8. Ao terminar, a GUI mostra o caminho da bibliotheca e um `biblio search` de exemplo que, copiado e colado, funciona.
...
11. Num Claude Code aberto numa pasta de projeto qualquer, apontar o caminho de **uma** das duas bibliothecas faz o agente buscar com `--lib` dela. A outra bibliotheca **não aparece** nos resultados.
12. **Portabilidade:** copie uma bibliotheca inteira para outro caminho (`xcopy`/`cp -r`), apague o registro (`~/.biblio/bibliothecas.txt`) e busque nela com `--lib <caminho novo>`. Funciona **sem nenhum comando de conserto**, e depois disso ela aparece em `biblio libs`. Nenhum arquivo dentro da pasta contém o caminho antigo — confira com `grep` no `CLAUDE.md`.
...
| `_HEADING` (o regex) | `normalize.py` | **Documento inteiro virando `01-parte.md`, `02-parte.md`** |

A última linha é uma previsão concreta, não uma hipótese: o fatiamento é guiado por heading Markdown, e **texto jurídico não tem heading**. "Art. 5º", "§ 1º" e "Inciso II" saem da conversão como parágrafo comum, então a lei inteira cai no caminho sem heading e vira partes numeradas — busca continua funcionando, estrutura se perde. Se o acervo for de direito, este é o primeiro lugar a olhar, e a correção é reconhecer `Art.`/`§` como heading em `normalize.py`, não mexer no `slice.py`.
