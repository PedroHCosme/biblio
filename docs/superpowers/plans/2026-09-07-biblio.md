...
Esperado: `8 passed`. Se `test_nao_mexe_na_hierarquia_quando_ja_comeca_em_nivel_um` falhar por causa do `\n` final acrescentado, ajuste o teste para comparar `entrada + "\n"` — a normalização garantir quebra final é comportamento desejado.
...
def test_documento_sem_heading_vira_partes_numeradas():
    ...
    md = ("y" * 1000 + "\n\n") * 60  # ~60 mil caracteres, passa dos 26 sufixos
...
PROMPT = """Voce recebe o inicio de um documento tecnico. Responda em portugues, \
exatamente neste formato, sem preambulo:

RESUMO: <uma frase dizendo o que o documento e e para que serve>
TERMOS: <8 a 12 termos tecnicos de busca, separados por virgula, sem numeracao>
...
SKILL = f"""---
name: biblioteca
description: Consultar a biblioteca de documentos tecnicos (normas, datasheets, \
papers) ingerida pelo biblio. Use sempre que a pergunta puder ser respondida por um \
documento do acervo em vez de conhecimento geral.
---

# Biblioteca de documentos

Acervo tecnico local, indexado pelo `biblio`. Grande demais para ler: o protocolo
abaixo existe para achar o paragrafo certo sem carregar o acervo.
...
Esta pasta e um acervo tecnico indexado. **Nao a leia por varredura** — sao
...
def instalar(avisar=print) -> Path:
    ...
    if not alvo.exists() or alvo.read_text(encoding="utf-8") != SKILL:
        alvo.write_text(SKILL, encoding="utf-8")
...
assert len(claude) < 2500, 'passou do orcamento de ~250 tokens (spec 7.4)'
from biblio.skill import SKILL
assert '--lib \"' not in SKILL, 'a skill global nao pode nascer presa a uma biblioteca'
" <caminho da lib>
```
...
> **Depende da Tarefa 0.2.** O bloco abaixo é a variante **numpy**. Se a medição escolheu `sqlite-vec`, troque `_buscar_vetorial` por uma consulta `vec0` e a tabela `vetores` por `CREATE VIRTUAL TABLE vetores USING vec0(id INTEGER PRIMARY KEY, emb float[{DIM}])`; o resto do arquivo não muda.
...
def substituir_documento(con: sqlite3.Connection, doc: str, chunks: list[dict],
    ...
    with con:
        ids = [r[0] for r in con.execute("SELECT id FROM chunks WHERE doc = ?", (doc,))]
        con.executemany("INSERT INTO chunks_fts(chunks_fts, rowid, texto, secao) "
                        "VALUES('delete', ?, '', '')", [(i,) for i in ids])
...
Esperado: `ok`. O `assert` do meio é o que garante que reprocessar não duplica.
...
Em `_processar_um`, depois de escrever as fatias e antes de `meta.escrever`:

```python
    avisar(f"{nome}: indexando")
    chunks = embed.chunks_do_documento(pasta)
    with db.conectar(biblioteca) as con:
        db.substituir_documento(con, nome, chunks, embed.vetorizar([c["texto"] for c in chunks]))
...
def test_identificador_exato_e_achado_pelo_fts(biblioteca_sintetica):
    ...
    assert achados[0]["doc"] == "manual-inversor"
...
def main(argv=None) -> int:
    ...
    if args.comando == "index":
        print(index.gerar(saida=args.out))
        return 0
...
def _processar(arquivos, pasta, destino, instalar_ollama, forcar):
    alvos = [f.name for f in (arquivos or [])] + ([pasta] if pasta else [])
    if not alvos:
