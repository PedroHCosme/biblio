...
BIBLIOTECA_PADRAO = Path.home() / "biblio"  # spec risco 6: padrao global, o atalho nao tem cwd util
...
def raiz(saida: Path | str | None = None) -> Path:
    return Path(saida) if saida else BIBLIOTECA_PADRAO
```
...
from biblio.paths import todas
...
from biblio.paths import BIBLIOTECA_PADRAO, conhecidas_biblioteca
...
def _processar(arquivos, pasta, destino, instalar_ollama, forcar):
    ...
    biblioteca = Path(destino or BIBLIOTECA_PADRAO)
    linhas, fila = [], []
    for alvo in alvos:
...
def subir(saida=None, share: bool = False) -> None:
    conhecidas = conhecidas_biblioteca()
    padrao = str(saida or (conhecidas[0] if conhecidas else BIBLIOTECA_PADRAO))

    with gr.Blocks(title="biblio") as tela:
        gr.Markdown("# biblio")
        destino = gr.Dropdown(
            label="Biblioteca",
            info="Onde os documentos vao. Digite outro caminho para comecar uma nova.",
...
**O botão diz "Adicionar documentos", não "Criar biblioteca"** (spec §7.2). O uso real é incremental; um botão que diz "criar" treina o usuário a fazer uma biblioteca nova a cada rodada, e aí ele tem cinco e procura em nenhuma. Criar outra continua sendo digitar outro caminho no campo do topo.
