...
```bash
biblio search "<a pergunta reescrita em termos do dominio>"
```
...
Esperado: o `INDEX.md` com um bloco por documento (incluindo o `.md`, cuja linha de procedência diz `md` em vez de `N pag`), e o `CLAUDE.md` dentro do orçamento.
...
`biblio shortcut` e a cada ingestao bem-sucedida. Cada biblioteca ainda ganha um
`CLAUDE.md` proprio, para o caso de a pasta ser copiada para outra maquina ou
aberta pelo Claude Desktop, pelo Cowork ou pelo ChatGPT.
...
11. Um PDF protegido por senha no meio de um lote é marcado `falhou` e **o lote continua**.

Os itens 1–10 são caminho feliz. **O item 11 é o que separa "processei 200 documentos" de "processei 12 e crashou"** — teste-o de propósito, com um PDF com senha de verdade.

**O item 10 é o teste de aceitação de verdade.** Os outros nove verificam que a ferramenta funciona; o 10 verifica que ela resolve o problema. Se o agente ignorar a skill e ler a pasta, o projeto falhou mesmo com todo o resto passando.
