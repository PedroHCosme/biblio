...
```python
"""Qual modelo de embedding cabe em CPU? Decide MODELO/DIM (risco #1 do spec).

Mede duas coisas: velocidade de indexação e se o modelo separa
portugues tecnico de vizinhanca proxima.
"""
import time
...
CORPUS = [
    "O comprimento de ancoragem basico e calculado pela resistencia de aderencia de calculo.",
    ...
    "Configuracao do inversor de frequencia para partida suave do motor trifasico.",
]
CONSULTA = "como calcular comprimento de ancoragem"
ESPERADO = 0  # a primeira frase e a resposta certa
...
for nome in CANDIDATOS:
    ...
    t = time.perf_counter()
    emb = m.encode(CORPUS * 40, batch_size=16, show_progress_bar=False)
    vazao = len(CORPUS) * 40 / (time.perf_counter() - t)

    pontos = util.cos_sim(m.encode(CONSULTA), m.encode(CORPUS))[0]
    acertou = int(pontos.argmax()) == ESPERADO

    print(f"{nome}\n  dim {emb.shape[1]}  carga {carga:.1f}s  "
          f"{vazao:.0f} chunks/s  acertou_top1={acertou}  margem={pontos.max()-pontos.sort(descending=True)[0][1]:.3f}\n")
```
...
Regra de decisão: **descarte qualquer modelo que erre o top-1** nesse corpus mínimo. Entre os que acertam, fique com o mais rápido. Se todos acertarem, `multilingual-e5-small` vence por velocidade e por caber em 384 dimensões.

Critério de corte duro: menos de 20 chunks/s em CPU torna a ingestão do acervo inviável — descarte.
...
def test_documento_sem_heading_vira_partes_numeradas():
    ...
    md = ("y" * 1000 + "\n\n") * 200  # ~200 mil caracteres; com TETO=8000 dá 29 fatias
...
def adicionar(alvo: Path | str, saida: Path | str | None = None, device: str = "auto",
    ...
    registrar(biblioteca)  # a busca precisa saber que esta biblioteca existe
...
Esperado: `{'ok': 2, 'pulado': 0, 'falhou': 0}`, as duas pastas com `_meta.yaml` e fatias, e o frontmatter do `.md` **sem o campo `paginas`** — origem sem páginas não inventa página.
...
PROMPT = """Voce recebe o inicio de um documento. Responda em portugues, \
...
def texto_claude_md(biblioteca: Path) -> str:
    """O mesmo protocolo, mas com `--lib` desta pasta ja preenchido.
...
O `--lib` abaixo restringe a busca a esta pasta. Se ela foi movida ou copiada de
outra maquina, rode `biblio --out . index` de dentro dela para corrigir o caminho.
Nao e preciso registrar a pasta: `--lib` funciona em qualquer copia que traga o
`.biblio.db` junto.

{PROTOCOLO.format(escopo=f' --lib "{caminho}"')}"""
...
def gerar(saida=None, resumir_pendentes: bool = True, avisar=print) -> Path:
    ...
    (biblioteca / "CLAUDE.md").write_text(skill.texto_claude_md(biblioteca),
                                          encoding="utf-8")
...
assert f'--lib \"{lib.resolve()}\"' in claude, 'CLAUDE.md nao restringe a propria pasta'
...
v2 = [dict(chunks[0], texto='cobrimento nominal da armadura')]
...
```python
    ...
    try:
        with con:  # `with` de sqlite3 e transacao, nao fechamento
...
def buscar(consulta: str, saida=None, top: int = 5, doc: str | None = None) -> list[dict]:
    ...
    for biblioteca in todas(saida):
        if not (biblioteca / db.ARQUIVO).exists():  # nao cria banco em pasta alheia
            if saida is None:
                continue
            # Pasta pedida de proposito (--lib, tipicamente vindo do CLAUDE.md de uma
            # copia com caminho velho). Silencio aqui vira "zero resultados" sem causa;
            # a mensagem diz o comando que conserta.
            raise SystemExit(
                f'{biblioteca}: nao e uma biblioteca biblio (falta {db.ARQUIVO}).\n'
                f'Se a pasta veio de outra maquina ou mudou de lugar, rode:\n'
                f'    biblio --out "{biblioteca}" index')
        con = db.conectar(biblioteca)
        try:
...
def main(argv=None) -> int:
    ...
    b.add_argument("--lib", help="restringe a uma biblioteca (padrao: todas)")
...
todas as bibliotecas registradas.

**Para um projeto usar so uma biblioteca**, aponte o caminho da pasta para o
agente, ou copie a pasta para dentro do projeto. O `CLAUDE.md` dela ja traz o
`--lib` correspondente, entao a busca fica restrita aquele acervo. Apontar e
melhor que copiar — copias divergem quando uma recebe documento novo. Se a pasta
mudar de lugar, rode `biblio --out . index` de dentro dela.
...
12. Um PDF protegido por senha no meio de um lote é marcado `falhou` e **o lote continua**.

Os itens 1–11 são caminho feliz. **O item 12 é o que separa "processei 200 documentos" de "processei 12 e crashou"** — teste-o de propósito, com um PDF com senha de verdade.
